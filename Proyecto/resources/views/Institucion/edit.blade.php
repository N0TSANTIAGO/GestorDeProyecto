<x-app-layout>
    ...
    <div class="py-12 px-6">
        <form action="{{ route('institucion.update', $institucion) }}" method="POST" class="bg-white p-6 rounded shadow-md max-w-xl">
            ...
            <div class="mb-4">
                <label class="block font-bold">Nombre de la institución</label>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="direccion" value="{{ old('direccion', $institucion->direccion) }}" class="w-full border p-2 rounded">
                @error('direccion')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
